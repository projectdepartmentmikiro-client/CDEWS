...
def process_qr_code(data, bbox=None):
    with scanned_qrs_lock:
        if data in scanned_qrs:
            return
        ...
        if bbox is not None:
            ...
            h, w = camera_qr.read()[1].shape[:2] if camera_qr.read()[0] else (480, 640)
            x_min, y_min, x_max, y_max = map(
                int, [max(0, x_min), max(0, y_min), min(w, x_max), min(h, y_max)]
            )
            ...
            if ret and frame is not None:
                ...
                with qr_display_lock:
                    global qr_display_mode, freeze_qr_image
                    qr_display_mode = "freeze"
                    freeze_qr_image = qr_crop.copy()
        sample_filename = safe_filename("sample", data)
        sample_path = os.path.join(SAMPLE_DIR, sample_filename)
        with open(CSV_FILE, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                [timestamp, crop_filename, data, sample_filename, "", "", "", "", ""]
            )
        with last_qr_lock:
            last_qr_data.update(
                {
        ...
        threading.Thread(
            target=capture_sample_after_delay, args=(sample_path, 3), daemon=True
        ).start()
...
def generate_frames_qr():
    global freeze_qr_image, qr_display_mode
    while True:
        ret, frame = camera_qr.read()
        ...
        if mode == "live":
            ...
            if data:
                bbox = (
                    bbox.astype(int).reshape(-1, 2)
                    if bbox is not None and len(bbox) > 0
                    else None
                )
                process_qr_code(data, bbox)
...
if __name__ == "__main__":
    ...
    threading.Thread(target=keyboard_qr_listener, daemon=True).start()
